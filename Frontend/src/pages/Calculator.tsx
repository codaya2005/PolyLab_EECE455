...
export default function CalculatorPage() {
  ...
  const modPolySafe = useMemo(() => {
    const fallback = IRRED_DEFAULTS[m] ?? 1;
    return modPoly && modPoly !== 0 ? modPoly : fallback;
  }, [m, modPoly]);

  const cfg: GFConfig = useMemo(() => ({ m, modPoly: modPolySafe }), [m, modPolySafe]);
  ...
  function onChangeM(next: number) {
    setM(next);
    const fallback = IRRED_DEFAULTS[next] ?? 1;
    setModPoly(IRRED_DEFAULTS[next] ?? fallback);
  }
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur p-6">
            ...
            <div className="mb-5">
              ...
              <Input
                value={hex(modPolySafe).slice(2)}
                onChange={(e) => {
                  const clean = e.target.value.replace(/[^0-9a-fA-F]/g, "");
                  const parsed = parseInt(clean || "0", 16) >>> 0;
                  // allow typing but never leave it at zero; fall back to default for m
                  setModPoly(parsed === 0 ? IRRED_DEFAULTS[m] ?? 1 : parsed);
                }}
                className="h-11 bg-slate-900/70 border-slate-700/70"
              />
              <div className="mt-1 text-xs text-slate-400">
                Polynomial: <span className="font-mono">{asPolyString(modPolySafe)}</span> · Default for m={m} is {hex(IRRED_DEFAULTS[m] ?? modPolySafe)}
              </div>
            </div>
