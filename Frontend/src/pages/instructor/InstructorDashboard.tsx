...
export default function InstructorDashboard() {
  ...
  const [mfaQrUrl, setMfaQrUrl] = React.useState<string | null>(null);
  ...
  const refresh = React.useCallback(async () => {
    ...
    try {
      ...
      if (!Array.isArray(data)) {
        setError("Unexpected classrooms response");
        setClasses([]);
      } else {
        setClasses(data);
      }
  ...
  React.useEffect(() => {
    ...
    async function buildQr() {
      ...
      try {
        ...
        const url = await QRCode.toDataURL(mfaOtpAuth);
        if (!cancelled) setMfaQrUrl(url);
      } catch (err) {
        if (!cancelled) {
          setMfaQrUrl(`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(mfaOtpAuth)}`);
        }
      }
    }
    buildQr();
  ...
  async function handleCreate(name: string, reset: () => void) {
    const trimmed = name.trim();
    if (!trimmed || creating) return;
    setCreating(true);
    setError(null);
    try {
      const cls = await createClassroom(trimmed);
      reset();
      await refresh();
  ...
  const triggerCreateMy = async () => handleCreate(myName, () => setMyName(""));
  const triggerCreateQuick = async () => handleCreate(quickName, () => setQuickName(""));
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur p-6">
            ...
            <div className="mb-4 flex gap-2">
              ...
              />
              <Button onClick={triggerCreateMy} disabled={creating} className="bg-cyan-500 text-slate-900 hover:bg-cyan-400">
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <Input
                    ...
                    className="bg-slate-900/60 border-slate-700"
                  />
                <Button onClick={triggerCreateQuick} disabled={creating} className="h-11 bg-cyan-500 text-slate-900 hover:bg-cyan-400">
                ...
                <div className="text-sm text-slate-300 space-y-2">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {mfaQrUrl ? (
                      ...
                      <div className="h-32 w-32 grid place-items-center rounded-lg border border-slate-800 bg-slate-900/60 p-1 text-xs text-slate-400">
                        Loading QR...
                      </div>
                    )}
                  </div>
