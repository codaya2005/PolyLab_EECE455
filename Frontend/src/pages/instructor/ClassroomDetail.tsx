...
function SubmissionsPanel({
  ...
  const grouped = (assignments ?? []).map((a) => ({
    assignment: a,
    label: `Asg ${asgNumberMap[a.id] ?? ""}`.trim(),
    subs: (submissions ?? []).filter((s) => String(s.assignment_id) === String(a.id)),
  }));
...
function SubmissionRow({ submission, relative }: { submission: Submission; relative: (iso: string) => string }) {
  ...
  const contentIsFile = typeof submission.content === "string" && submission.content.startsWith("/uploads/");
  const contentUrl = contentIsFile ? `${AUTH_BASE_URL}${submission.content}` : null;
      ...
      <div className="flex justify-between text-sm text-slate-200">
        <div>
          ...
          {contentUrl && (
            ...
            >
              <ExternalLink className="h-3.5 w-3.5" /> View submission file
            </a>
          )}
          {submission.content && (
            <div className="mt-1 text-xs text-slate-300 whitespace-pre-wrap break-words max-w-2xl">
              {submission.content}
            </div>
          )}
        </div>
