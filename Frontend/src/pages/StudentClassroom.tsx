            ...
            <TabsContent value="assignments" className="mt-4 space-y-4">
              {assignments.length === 0 ? (
                ...
                assignments.map((a) => {
                  ...
                  const submittedList = mySubs[a.id] ?? [];
                  return (
                        ...
                        <div>
                          ...
                          {submittedList.length > 0 && (
                            <div className="mt-2 space-y-1 text-xs text-emerald-300">
                              {submittedList.map((s) => {
                                const isFile = typeof s.content === "string" && s.content.startsWith("/uploads/");
                                const url = isFile ? `${AUTH_BASE_URL}${s.content}` : null;
                                return (
                                  <div key={s.id} className="flex flex-wrap items-center gap-2">
                                    <span>Submitted {new Date(s.submitted_at).toLocaleString()}</span>
                                    {s.grade !== null && s.grade !== undefined && <span>• Grade: {s.grade}</span>}
                                    {url ? (
                                      <a
                                        href={url}
                                        target="_blank"
                                      ...
                                      <span className="text-slate-200 whitespace-pre-wrap break-words">{s.content}</span>
                                    )}
                                  </div>
                                );
                              })}
                      ...
                      return (
                        <div key={s.id} className="rounded-lg border border-slate-800 bg-slate-900/50 p-4">
                          ...
                          <div className="mt-1 text-sm">
                            {url ? (
                              <a href={url} target="_blank" rel="noopener noreferrer" className="text-cyan-300 hover:text-cyan-200">
                                View file
                              </a>
                            ) : (
                              <span className="text-slate-200 whitespace-pre-wrap break-words">{s.content}</span>
